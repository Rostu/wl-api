...
wlapi.baseform("Wälder",function(err,callback){
    console.log(callback); --> [ [ 'Wald', 'N' ] ]
    }
); 
```
if there is no search reult for your query you get null
```
wlapi.baseform("Wildssdflder",function(err,callback){
...
```
wlapi.baseform("Bäume",function(err,callback){
    console.log(callback); --> [ [ 'Baum', 'N' ] ]
    }
);  
```
...
```
wlapi.frequencies("Esel",function(err,callback){
    console.log(callback); --> [ '3524', '12' ]
    }
);  
```
...
```
wlapi.domain("Esel",function(err,callback){
    console.log(callback);--> [ 'Spezielle Zoologie','Säugetiere','Nutztiere','pejorativ','Menschen psychisch' ]
    }
);
```
...
```
wlapi.wordforms("laufen",5,function(err,callback){
console.log(callback);--> [ 'läuft', 'laufen', 'lief', 'gelaufen', 'liefen' ]
    }
);
```
...
```
wlapi.synonyms("Esel",5,function(err,callback){
console.log(callback);-->   [ [ 'Depp', 's' ],
                            [ 'Dummkopf', 's' ],
                           [ 'Grauchen', 'S' ],
                           [ 'Langohr', 'S' ],
                           [ 'Grautier', 'S' ] ]
...
```
wlapi.wordforms("Esel",5,function(err,callback){
console.log(callback);-->[ 'Pferd', 'Esel', 'Rind', 'Kamel', 'Büffel' ]
    }
);
```
...
```
wlapi.sentences("Bücher", 2 , function(err,callback){
    console.log(callback);-->[ [ '40587352',
...
```
wlapi.left_neighbours("Esel",5, function(err,callback){
  console.log(callback);-->[ [ 'Ochs', 'Esel', '40' ],
...
```
wlapi.right_neighbours("Esel",5, function(err,callback){
    console.log(callback); -->[ [ 'Esel', 'reitend', '23' ],
                              [ 'Esel', 'streck', '23' ],
                               [ 'Esel', 'Ziegen', '19' ],
                               [ 'Esel', 'reiten', '17' ],
                               [ 'Esel', 'I-Aah', '14]]
...
```
wlapi.similarity("Esel",5, function(err,callback){
    console.log(callback);  -->[ [ 'Esel', 'Rappen', '18' ],
                              [ 'Esel', 'Gaul', '17' ],
                              [ 'Esel', 'Pferd', '17' ],
                              [ 'Esel', 'Roß', '15' ],
                              [ 'Esel', 'andern', '14' ] ]
...
```
wlapi.experimental_synonyms("Esel",5, function(err,callback){
    console.log(callback);  --> [ [ 'Grautier', 'v' ],
                                [ 'Langohr', 'v' ],
                                [ 'Lasttier', 'v' ],
                                [ 'Lama', 'v' ],
                                [ 'Muli', 'v' ] ]
...
```
wlapi.right_collocation_finder("Esel","V",5, function(err,callback){
    console.log(callback);-->   [ 'Esel', 'anbinden', 'V' ],
                                  [ 'Esel', 'anreiten', 'V' ],
                                  [ 'Esel', 'begegnen', 'V' ],
                                  [ 'Esel', 'bepacken', 'V' ],
                                  [ 'Esel', 'beschimpfen', 'V' ]]
...
```
wlapi.left_collocation_finder("Esel","A",7, function(err,callback){
    console.log(callback);--> [ 'Golden', 'A', 'Esel' ],
                              [ 'alt', 'A', 'Esel' ],
                              [ 'andalusisch', 'A', 'Esel' ],
                              [ 'beladen', 'A', 'Esel' ],
                              [ 'bepackt', 'A', 'Esel' ],
...
```
wlapi.cooccurrences("Esel",200,5, function(err,callback){
    console.log(callback);-->[  [ 'Esel', 'Ochs', '484' ],
                                [ 'Esel', 'und', '364' ],
                                [ 'Esel', 'ein', '213' ] ]
    }
);
